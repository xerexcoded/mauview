...
impl<'a>Config<'a>{
    pub fn new(matches: &'a ArgMatches) -> Config<'a>{
        ...
        let frame_duration=if matches.is_present("frame-per-second"){
            let frame_rate= value_t!(matches,"frame-per-second",f32).unwrap_or_else(|e| e.exit());
            Some(Duration::from_secs_f32(1.0/frame_rate))
        }else{None};
        Config{
            files,loop_gif,name:matches.is_present("name"),
            mirror:matches.is_present("mirror"),
            recursive:matches.is_present("recursive"),
            static_gif,viuer_config,frame_duration,
        }
    ...
    pub fn test_config() -> Config<'a>{
        Config{
            ...
            viuer_config:ViuerConfig{
                ...
                use_kitty:false,  //just use kitty , 🐈
                ..Default::default()
            },
            frame_duration:None,
        }
